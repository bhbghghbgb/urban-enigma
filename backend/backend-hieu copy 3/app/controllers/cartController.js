...
exports.addToCart = async (req, res) => {
    try {
        ...
        if (!cart) {
            ...
            const existingProduct = cart.products.find(p => p.product._id == req.body.productId);
            if (existingProduct) {
...
exports.deleteProductOfCart = async (req, res) => {
    try {
        ...
        var isProductInCart = cart.products.find(product => product.product._id.toString() == req.body.productId);
        if (!isProductInCart) {
            ...
            cart.save();
            res.status(201).json({ message: 'Product has been removed from the cart' });
            return;
        }
    } catch (err) {
        res.status(500).json({ message: err.message });
...
exports.increaseProductOfCart = async (req, res) => {
    try {
        ...
        const product = cart.products.find(p => p.product._id.toString() === req.params.productId);
        if (!product) {
...
exports.decreaseProductOfCart = async (req, res) => {
    try {
        ...
        const productIndex = cart.products.findIndex(p => p.product._id.toString() === req.body.product);
...
exports.resetCart = async (req, res) => {
    try {
        ...
        await Cart.updateOne({ user: customer }, { $set: { total: 0, products: [] } })
