...
exports.findAvailableStaff = async (req, res) => {
    try {
        const deliveringDeliveries = await Delivery.find({ status: 'delivering' });
        const deliveringStaffIds = deliveringDeliveries.map(delivery => delivery.staff);
        const availableStaff = await Staff.find({ _id: { $nin: deliveringStaffIds } });
...
exports.getOrdersByStaff = async (req, res) => {
    try {
        const staff = await Staff.findOne({ _id: req.user }).populate('commonuser.account');
        if (!staff) {
            res.status(404).json({ message: 'User not found' });
            return;
        }
        const deliveries = await Delivery.findOne({ staff: staff, status: 'delivering' });
        if(deliveries) {
            const orderIds = deliveries.order;
            const orders = await Order.findOne({ _id: orderIds }).populate({
                path: 'user',
                model: 'Customer',
                populate: {
...
exports.findOrderOfStaff = async (req, res) => {
    try {
        ...
        const deliveries = await Delivery.find({ staff: staff, status: 'delivered' });
