...
exports.createStaff = async (req, res) => {
    try {
        const { name, gender, dateOfBirth, phone, account, address, position } = req.body;
...
exports.createCustomer = async (req, res) => {
    try {
        const { name, gender, dateOfBirth, phone, account, qrCode, membershipPoint } = req.body;
...
exports.updateCustomer = async (req, res) => {
    try {
        const { name, gender, dateOfBirth, phone, qrCode, membershipPoint } = req.body;
        ...
        await customer.save();
        res.status(200).json({ message: 'Staff updated successfully', customer });
        return;
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
